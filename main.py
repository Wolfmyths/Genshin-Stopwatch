...
class addTimer(qtw.QDockWidget):
    def __init__(self, parent=None | qtw.QMainWindow):
        ...
        self.topicSelectionDict = {
            ...
            "Stamina": {"durations": "Stamina"},
            "Parametric Transformer": {"durations": tuple(["7 Days"])},
            "Respawns": {"durations": ("12 Hours", "1 Day", "2 Days", "3 Days")},
            "Expedition": {"durations": ("4 Hours", "8 Hours", "12 Hours", "20 Hours")},
            ...
            "Fishing": {"durations": ("1 Day", "3 Days")},
            "Custom": {"durations": "Custom"},
        }
        ...
        self.topicDropDown.currentTextChanged.connect(
            lambda topic: self.dropDownSelected(topic)
        )
        ...
        self.rCRateValuesDict = {
            k: v
            for (k, v) in zip(
                self.topicSelectionDict["Realm Currency"]["durations"],
                (4, 8, 16, 20, 22, 24, 26, 28, 30),
            )
        }
        ...
        self.rCFriendshipValuesDict = {
            k: v
            for (k, v) in zip(
                self.topicSelectionDict["Realm Companionship XP"]["durations"],
                range(2, 6),
            )
        }
    ...
    def startStopWatch(self):
        ...
        percentToMinutes: callable[[str], str] = lambda rate: round(
            (float(rate) / 100) * 60
        )
        calculateDuration: callable[[int, int], str] = lambda maxStorage, rate: str(
            round(maxStorage / rate, 2)
        ).split(".")
        ...
        match timeObject:
            ...
            case "Custom":
                days: str | int = (
                    self.lineEdit1.text() if self.lineEdit1.text().isdigit() else 0
                )
                hours: str | int = (
                    self.lineEdit2.text() if self.lineEdit2.text().isdigit() else 0
                )
                minutes: str | int = (
                    self.lineEdit3.text() if self.lineEdit3.text().isdigit() else 0
                )
        ...
        centralWidget_: centralWidget = self.parent().findChild(
            qtw.QWidget, "central widget"
        )
    ...
    def hideEvent(self, a0: qtg.QHideEvent) -> None:
        if self.parent().isHidden():
            ...
            addtimerButton: qtw.QAction = toolbar.findChild(
                qtw.QAction, "addTimerButton"
            )
...
class optionsDock(qtw.QDockWidget):
    def __init__(self, parent=None | qtw.QMainWindow):
        ...
        self.appOnCloseCheckbox.setChecked(
            config["OPTIONS"].getboolean("shutdown app on close")
        )
        self.appOnCloseCheckbox.clicked.connect(lambda: self.settingChanged(True))
        self.appOnCloseCheckbox.setToolTip(
            "The app will close and not run in the background."
        )
        # Form Row
        ...
        self.showOnOpenCheckbox.setChecked(
            config["OPTIONS"].getboolean("show on startup")
        )
        self.showOnOpenCheckbox.clicked.connect(lambda: self.settingChanged(True))
        self.showOnOpenCheckbox.setToolTip(
            "Program will automatically show or hide when starting."
        )
        # Form Row
        ...
        self.notifyCheckbox.setChecked(
            config["OPTIONS"].getboolean("desktop notifications")
        )
        self.notifyCheckbox.clicked.connect(lambda: self.settingChanged(True))
        self.notifyCheckbox.setToolTip(
            "A windows desktop notification will appear when a stopwatch finishes."
        )
        # Form Row
...
class toolbar(qtw.QToolBar):
    def __init__(self, parent=None | qtw.QMainWindow):
        ...
        self.addTimerButton.setChecked(
            config["QOL"].getboolean("addtimer open on startup")
        )
        self.addTimerButton.triggered.connect(
            lambda: self.button_Clicked(
                "addTimerDockWidget", self.addTimerButton.isChecked()
            )
        )
        ...
        self.optionsButton.setChecked(
            config["QOL"].getboolean("settings open on startup")
        )
        self.optionsButton.triggered.connect(
            lambda: self.button_Clicked(
                "optionsDockWidget", self.optionsButton.isChecked()
            )
        )
        self.addAction(self.optionsButton)

    def button_Clicked(self, dockObjectName: str, buttonisChecked: bool):
        addDockWidget: qtw.QDockWidget = self.parent().findChild(
            qtw.QDockWidget, dockObjectName
        )
...
class centralWidget(qtw.QWidget):
    ...
    def addStopWatch(
        ...
        name: str,
        startDuration: datetime.timedelta,
        color: str,
        notepadContents: str = "",
        index: int | None = None,
        save: bool = True,
        ...
        if startDuration >= datetime.timedelta(
            hours=24
        ):  # startDuration gives the days but not in total hours, needed so that loadSaveData() can work
            parent.setProperty(
                "originalDuration",
                f"{startDurationDays * 24}:{startDurationMinutes}:00",
            )
        ...
        resetButton.clicked.connect(
            lambda: self.resetTimer(
                timeObject, name, startDuration, id_, color, notes=notepad.toPlainText()
            )
        )
        frameLayout.addWidget(resetButton, 2, 0, alignment=Qt.AlignBottom)

        finishedDate = datetime.datetime.now() + duration
        finishedDateLabel = qtw.QLabel(
            f'Finished on: {finishedDate.strftime("%B %d @ %I:%M %p")}'
        )
        ...
        parent.setProperty(
            "finishedTime",
            datetime.datetime.strftime(finishedTime, "%Y-%m-%d %H:%M:%S"),
        )

        difference = finishedTime - currentTime

        resetButton: qtw.QPushButton = parent.findChild(
            qtw.QPushButton, f"{id_}resetButton"
        )
        countDownLabel: qtw.QLabel = parent.findChild(
            qtw.QLabel, f"{id_}CountDownLabel"
        )
...
class window(qtw.QMainWindow):
    def __init__(self):
        ...
        self.resize(
            config["WINDOW SIZE"].getint("width"),
            config["WINDOW SIZE"].getint("height"),
        )
    ...
    def loadSaveData(self):
        ...
        for timerID in data.sections():
            ...
            timeFinished = timeFinished - datetime.datetime.today().replace(
                microsecond=0
            )
            ...
            originalDuration = datetime.timedelta(
                hours=int(originalDuration[0]), minutes=int(originalDuration[1])
            )
    ...
    def saveData(self):
        ...
        for qtObject in self.central.findChildren(qtw.QFrame):
            ...
            if (
                len(objectName) == 13
            ):  # Length of a stopwatch's name which is their id() value
                data[objectName] = {
                    "name": qtObject.findChild(
                        qtw.QLabel, f"{objectName}nameLabel"
                    ).text(),
...
class trayMen(qtw.QMenu):
    def __init__(self):
        ...
        openOrClose = (
            "Close" if config["OPTIONS"].getboolean("show on startup") else "Open"
        )
